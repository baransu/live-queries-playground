...
const TodoRenderer = (props: { todo: TodoFragment }): React.ReactElement => {
  const [isEditing, setIsEditing] = React.useState(false);
  const [, deleteTodo] = useDeleteMutation();
  const [, changeTodo] = useChangeContentMutation();
  const [, toggleIsComplete] = useToggleIsCompletedMutation();
...
const TodoListRenderer = (props: { data: TodosQuery }): React.ReactElement => {
  return (
...
export const TodoApplication = (): React.ReactElement => {
  ...
  const [, addTodo] = useAddMutation();

  const handleTextInputSave = (content: string) => {
    addTodo({ id: randomId(), content });
  };
